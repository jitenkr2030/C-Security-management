...
void MainWindow::buildUI()
{
    ...
    auto ph = [](const QString& title) {
        auto* w = new QWidget; auto* l = new QVBoxLayout(w); l->addStretch();
        auto* label = new QLabel(title + "\n\nComing Soon"); label->setObjectName("PageTitle"); label->setAlignment(Qt::AlignCenter); l->addWidget(label); l->addStretch(); return w;
    };
...
QWidget* MainWindow::createSidebar()
{
    ...
    auto* logo = new QLabel("SGMS"); logo->setObjectName("SidebarTitle"); sidebarLayout->addWidget(logo);
    auto* subtitle = new QLabel("Security Guard Manager"); subtitle->setObjectName("SidebarSubtitle"); sidebarLayout->addWidget(subtitle);

    auto addDivider = [&]() { auto* div = new QFrame; div->setObjectName("SidebarDivider"); div->setFrameShape(QFrame::HLine); sidebarLayout->addWidget(div); };
    auto addSection = [&](const QString& text) { addDivider(); auto* label = new QLabel(text); label->setObjectName("SectionTitle"); label->setStyleSheet("padding: 8px 16px 4px 16px; color: #3D4654;"); sidebarLayout->addWidget(label); };
    auto makeBtn = [&](const QString& text, int index) -> QPushButton* {
        auto* btn = new QPushButton(text); btn->setCheckable(true); btn->setCursor(Qt::PointingHandCursor); sidebarLayout->addWidget(btn);
        connect(btn, &QPushButton::clicked, this, [this, btn, index]() { m_stack->setCurrentIndex(index); setActiveNavButton(btn); }); return btn;
    };

    sidebarLayout->addSpacing(8);
    addSection("MAIN");
    m_btnDashboard = makeBtn("  Dashboard", 0); m_btnGuards = makeBtn("  Guards", 1);
    m_btnClients = makeBtn("  Clients", 2); m_btnSites = makeBtn("  Sites", 3);
    addSection("OPERATIONS");
    m_btnAttendance = makeBtn("  Attendance", 4); m_btnDuty = makeBtn("  Duty Allocation", 5);
    m_btnLeave = makeBtn("  Leave", 6); m_btnSalary = makeBtn("  Salary", 7);
    addSection("RESOURCES");
    m_btnUniform = makeBtn("  Uniform", 8); m_btnEquipment = makeBtn("  Equipment", 9);
    m_btnVisitors = makeBtn("  Visitors", 10); m_btnVehicles = makeBtn("  Vehicles", 11);
    addSection("ADMIN");
    m_btnIncidents = makeBtn("  Incidents", 12); m_btnTraining = makeBtn("  Training", 13);
    m_btnDocuments = makeBtn("  Documents", 14); m_btnReports = makeBtn("  Reports", 15);
    m_btnSearch = makeBtn("  Search", 16); m_btnBackup = makeBtn("  Backup", 17);
...
{
    QList<QPushButton*> allBtns = { m_btnDashboard, m_btnGuards, m_btnClients, m_btnSites, m_btnAttendance, m_btnDuty, m_btnLeave, m_btnSalary, m_btnUniform, m_btnEquipment, m_btnVisitors, m_btnVehicles, m_btnIncidents, m_btnTraining, m_btnDocuments, m_btnReports, m_btnSearch, m_btnBackup, m_btnSettings };
