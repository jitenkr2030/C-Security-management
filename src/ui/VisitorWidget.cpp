...
VisitorWidget::VisitorWidget(QWidget* parent) : QWidget(parent)
{
    buildUI();
    loadVisitors();
}

void VisitorWidget::buildUI()
{
    auto* mainLayout = new QVBoxLayout(this);
    mainLayout->setContentsMargins(32, 28, 32, 28);
    mainLayout->setSpacing(16);

    auto* titleLabel = new QLabel("Visitor Register");
    titleLabel->setObjectName("PageTitle");
    mainLayout->addWidget(titleLabel);

    auto* subtitle = new QLabel("Log visitor entries and exits at all sites");
    ...
    m_addBtn = new QPushButton("+ New Visitor");
    m_addBtn->setObjectName("PrimaryButton");
    m_addBtn->setFixedSize(130, 36);
    m_addBtn->setCursor(Qt::PointingHandCursor);
    connect(m_addBtn, &QPushButton::clicked, this, &VisitorWidget::addVisitor);
    ...
    connect(m_editBtn, &QPushButton::clicked, this, &VisitorWidget::editVisitor);
    ...
    connect(m_exitBtn, &QPushButton::clicked, this, &VisitorWidget::markExit);
    ...
    connect(m_deleteBtn, &QPushButton::clicked, this, &VisitorWidget::deleteVisitor);
    ...
    connect(m_exportBtn, &QPushButton::clicked, this, &VisitorWidget::exportCSV);
    ...
    m_searchEdit->setPlaceholderText("Search by name, mobile, vehicle...");
    m_searchEdit->setClearButtonEnabled(true);
    connect(m_searchEdit, &QLineEdit::textChanged, this, &VisitorWidget::filterVisitors);
    ...
    connect(m_siteFilter, &QComboBox::currentTextChanged, this,
            [this](const QString&) { filterVisitors(m_searchEdit->text()); });
    ...
    m_dateFilter = new QDateEdit;
    m_dateFilter->setCalendarPopup(true);
    m_dateFilter->setDisplayFormat("yyyy-MM-dd");
    m_dateFilter->setDate(QDate::currentDate());
    m_dateFilter->setSpecialValueText("All Dates");
    m_dateFilter->setFixedWidth(140);
    connect(m_dateFilter, &QDateEdit::dateChanged, this, &VisitorWidget::filterByDate);
    filterRow->addWidget(m_dateFilter);
    ...
    QStringList cols = {"ID", "Site", "Name", "Mobile", "ID Proof", "ID Number",
                        "Purpose", "To Meet", "Entry Time", "Exit Time",
    ...
    m_table->setColumnWidth(9, 140);
    m_table->setColumnWidth(10, 100);
    m_table->setColumnWidth(11, 70);
    m_table->horizontalHeader()->setStretchLastSection(true);

    connect(m_table, &QTableWidget::cellDoubleClicked, this, &VisitorWidget::editVisitor);

    mainLayout->addWidget(m_table, 1);
}

void VisitorWidget::loadVisitors()
{
    auto& db = DatabaseManager::instance();
    ...
    int count = 0;
    auto cc = db.execute("SELECT COUNT(*) FROM Visitors");
    ...
    while (query.next()) {
        ...
        setItem(4, query.value("id_proof").toString());
        setItem(5, query.value("id_number").toString());
        setItem(6, query.value("purpose").toString());
        setItem(7, query.value("whom_to_meet").toString());
        setItem(8, query.value("entry_time").toString());

        QString exitTime = query.value("exit_time").toString();
        setItem(9, exitTime.isEmpty() ? "-" : exitTime);
        setItem(10, query.value("vehicle_no").toString());
        ...
        setItem(12, query.value("notes").toString());
    ...
    m_summaryLabel->setText(
        QString("Today: %1 total | Currently Inside: %2 | Exited: %3")
            .arg(row).arg(inside).arg(exited)
    );
}

void VisitorWidget::refresh()
{
    loadVisitors();
}

void VisitorWidget::addVisitor()
{
    VisitorDialog dlg(this, -1);
    if (dlg.exec() == QDialog::Accepted) loadVisitors();
}

void VisitorWidget::editVisitor()
{
    auto items = m_table->selectedItems();
    if (items.isEmpty()) {
        QMessageBox::information(this, "No Selection", "Select a visitor entry to edit.");
        return;
    }
    int id = m_table->item(items.first()->row(), 0)->text().toInt();
    VisitorDialog dlg(this, id);
    if (dlg.exec() == QDialog::Accepted) loadVisitors();
}

void VisitorWidget::markExit()
{
    auto items = m_table->selectedItems();
    if (items.isEmpty()) {
        QMessageBox::information(this, "No Selection", "Select a visitor to mark exit.");
        return;
    }
    int row = items.first()->row();
    int id = m_table->item(row, 0)->text().toInt();
    QString name = m_table->item(row, 2)->text();
    QString status = m_table->item(row, 11)->text();

    if (status == "Exited") {
        QMessageBox::information(this, "Already Exited",
            QString("%1 has already exited.").arg(name));
        return;
    }

    auto result = QMessageBox::question(this, "Mark Exit",
        QString("Mark exit for \"%1\"?\nTime: %2").arg(name, QTime::currentTime().toString("HH:mm")),
    ...
    if (result == QMessageBox::Yes) {
        ...
        db.executeNonQuery(
            "UPDATE Visitors SET exit_time = :time WHERE id = :id",
            {{":time", exitTime}, {":id", id}}
        );
        loadVisitors();
    }
...
void VisitorWidget::deleteVisitor()
{
    auto items = m_table->selectedItems();
    if (items.isEmpty()) {
        QMessageBox::information(this, "No Selection", "Select a visitor entry to delete.");
        return;
    }
    int row = items.first()->row();
    int id = m_table->item(row, 0)->text().toInt();
    ...
    auto result = QMessageBox::question(this, "Delete",
        QString("Delete visitor entry for \"%1\"?").arg(name),
        QMessageBox::Yes | QMessageBox::No, QMessageBox::No);

    if (result == QMessageBox::Yes) {
        auto& db = DatabaseManager::instance();
        db.executeNonQuery("DELETE FROM Visitors WHERE id = :id", {{":id", id}});
        loadVisitors();
    }
}

void VisitorWidget::filterVisitors(const QString& text)
{
...
void VisitorWidget::filterByDate()
{
    // Re-apply all filters including date
    filterVisitors(m_searchEdit->text());
}

void VisitorWidget::exportCSV()
{
    if (m_table->rowCount() == 0) {
        QMessageBox::information(this, "No Data", "No visitor data to export.");
        return;
    }

    QDir().mkpath(QCoreApplication::applicationDirPath() + "/reports");
    QString filePath = QFileDialog::getSaveFileName(
        this, "Export Visitor Report",
        QCoreApplication::applicationDirPath() + "/reports/visitors_" +
    ...
    QMessageBox::information(this, "Export Successful",
        QString("Visitor report exported to:\n\n%1").arg(filePath));
}
